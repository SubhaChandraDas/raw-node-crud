...
handlers._users.get = function(data, callBack) {
    ...
    const phone = typeof(data.queryStringObject.phone.trim()) === "string" && data.queryStringObject.phone.trim().length === 10 ? data.queryStringObject.phone.trim() : false;
    if (phone) {
        fileOperations.read('users', phone, function(error, data) {
            if (!error && data) {
                const { hashedPassword, ...userResponse } = data;
                callBack(200, userResponse)
            } else {
